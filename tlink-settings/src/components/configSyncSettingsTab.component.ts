...
import { BaseComponent as CoreBaseComponent, ConfigService, PromptModalComponent, HostAppService, PlatformService, NotificationsService, TranslateService } from 'tlink-core'
...
export class ConfigSyncSettingsTabComponent extends BaseComponent {
    ...
    openSyncHost () {
        if (this.config.store.configSync.host === 'https://api.tlink.sh') {
            this.platform.openExternal('https://app.tlink.sh')
        } else {
    ...
    openTlinkWebInfo () {
        this.platform.openExternal('https://github.com/Eugeny/tlink-web')
    }
