...
import { applyOutputHighlighting, compileOutputHighlightRules, OutputHighlightRule } from '../outputHighlighting'
...
const ALT_SCREEN_SEQUENCE_REGEX = new RegExp(ALT_SCREEN_SEQUENCES.map(x => x.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'), 'g')
const ALT_SCREEN_SEQUENCE_PREFIXES = new Set<string>(
    ALT_SCREEN_SEQUENCES.flatMap(seq => {
        const prefixes: string[] = []
        for (let i = 1; i < seq.length; i++) {
            prefixes.push(seq.slice(0, i))
        }
        return prefixes
    }),
)
const ALT_SCREEN_SEQUENCE_MAX_LENGTH = Math.max(...ALT_SCREEN_SEQUENCES.map(seq => seq.length))
...
export class BaseTerminalTabComponent<P extends BaseTerminalProfile> extends BaseTabComponentRuntime implements OnInit, OnDestroy {
    ...
    get showPaneClose (): boolean {
        // Used by the split-pane header/toolbar close button.
    ...
    // Coalesce high-volume output to avoid UI stalls during bursty sessions.
    private pendingOutput = ''
    private pendingOutputFrame?: number
    ...
    private queueOutput (data: string): void {
        ...
        if (this.pendingOutput.length >= this.pendingOutputMax) {
            this.flushPendingOutput()
            return
        }
        if (this.pendingOutputFrame === undefined) {
            this.pendingOutputFrame = window.requestAnimationFrame(() => {
                this.pendingOutputFrame = undefined
                this.flushPendingOutput()
            })
        }
    }
    ...
    async paste (): Promise<void> {
        ...
        data = this.stripAnsiSequences(data)
        ...
        if (this.config.store.terminal.trimWhitespaceOnPaste && data.endsWith('\r') && !data.slice(0, -1).includes('\r')) {
            // Ends with a single trailing newline (already converted to \r above)
    ...
    async closePane (): Promise<void> {
        ...
        if (this.parent.getAllTabs().length < 2) {
            return
        }
    ...
    protected attachSessionHandlers (destroyOnSessionClose = false): void {
        ...
        this.attachSessionHandler(this.session.output$, data => {
            if (this.enablePassthrough) {
                if (this.config.store.terminal.disableAlternateScreen) {
                    data = this.stripAlternateScreenSequences(data)
                    if (!data) {
                        return
                    }
                }
                data = this.applyOutputHighlightingToData(data)
        ...
        this.attachSessionHandler(this.session.activity$, () => {
            if (!this.hasFocus) {
                this.displayActivity()
            }
        })
    ...
    private applyOutputHighlightingToData (data: string): string {
        const config = this.config.store.terminal.outputHighlighting
        if (!config?.enabled) {
            return data
        }

        const key = JSON.stringify(config)
        if (this.outputHighlightingKey !== key) {
            this.outputHighlightingRules = compileOutputHighlightRules(config)
            this.outputHighlightingKey = key
        }

        return applyOutputHighlighting(
            data,
            this.outputHighlightingRules,
            config.skipIfAnsiPresent,
        )
    }

    private stripAlternateScreenSequences (data: string): string {
        const combined = this.altScreenSequenceBuffer + data
        this.altScreenSequenceBuffer = ''
        let stripped = combined.replace(ALT_SCREEN_SEQUENCE_REGEX, '')
        const maxSuffixLength = Math.min(ALT_SCREEN_SEQUENCE_MAX_LENGTH - 1, stripped.length)
        for (let i = maxSuffixLength; i > 0; i--) {
            const suffix = stripped.slice(-i)
            if (ALT_SCREEN_SEQUENCE_PREFIXES.has(suffix)) {
                this.altScreenSequenceBuffer = suffix
    ...
    private stripAnsiSequences (data: string): string {
        if (!data.includes('\x1b') && !data.includes('\x9b')) {
            return data
        }
        return data.replace(ANSI_SEQUENCE_REGEX, '')
    }
