...
import { ConfigService, getCSSFontFamily, getWindows10Build, HostAppService, HotkeysService, Platform, PlatformService, ThemesService } from 'tlink-core'
...
export class XTermFrontend extends Frontend {
    ...
    copySelection (): void {
        ...
        const hasLineBreaks = /[\r\n]/.test(text)
        const allowHTML = this.configService.store.terminal.copyAsHTML && text.length < 1024 * 32 && !hasLineBreaks
        this.platformService.setClipboard(allowHTML ? {
            text,
