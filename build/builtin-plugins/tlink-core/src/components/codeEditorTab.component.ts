...
export class CodeEditorTabComponent extends BaseTabComponent implements AfterViewInit {
    ...
    private readonly topologyNodeWidthPx = 80
    private readonly topologyNodeHeightPx = 56
    ...
    getTopologyTextHeight (item: TopologyTextModel): number|null {
        ...
        const widthRaw = Number(item.width)
        const width = Number.isFinite(widthRaw) ? Math.max(100, widthRaw) : 80
    ...
    async exportTopologyAsImage (): Promise<void> {
        ...
        const allY: number[] = []
        for (const n of nodes) {
            allX.push(n.x, n.x + (n.width || 80))
            allY.push(n.y, n.y + (n.height || 56))
        }
        ...
        const getCenter = (id: string | undefined, kind: string | undefined): { x: number, y: number } | null => {
            ...
            if (n) return { x: n.x + (n.width || 80) / 2 + offsetX, y: n.y + (n.height || 56) / 2 + offsetY }
            return null
        }
    ...
    async openSampleFile (kind: 'json'|'yaml'|'topology'|'python'): Promise<void> {
        if (!(await this.ensureEditor())) {
            return
        }
        let name: string
        ...
        case 'python':
            ...
            content = [
                '#!/usr/bin/env python3',
                '"""',
                'Network Scanner & Health Check Utility',
                '',
                'Scans a subnet for active hosts, checks port availability,',
                'and generates a network health report.',
                '"""',
                '',
                'import socket',
                'import subprocess',
                ...
                '',
                '',
                'def scan_host(ip: str, ports: List[int] = None) -> HostResult:',
                '    """Scan a single host: ping, resolve hostname, check ports."""',
                '    if ports is None:',
                ...
                '        result.open_ports = [p for p in ports if check_port(ip, p)]',
                '',
                '    return result',
                '',
                '',
                'def scan_subnet(subnet: str, max_workers: int = 50) -> List[HostResult]:',
                ...
                '            if result.is_alive:',
                '                port_info = ", ".join(',
                '                    f"{p} ({COMMON_PORTS.get(p, \'unknown\')})"',
                '                    for p in result.open_ports',
                '                )',
                '                print(',
                '                    f"  [+] {result.ip:15s}  {result.hostname or \'\':30s}  "',
                '                    f"{result.latency_ms:6.1f}ms  Ports: {port_info or \'none\'}"',
                '                )',
                ...
                '',
                '',
                'def generate_report(results: List[HostResult]) -> dict:',
                ...
                '            sum(r.latency_ms for r in alive) / len(alive), 2',
                '        ) if alive else 0,',
                '        "hosts": [asdict(r) for r in alive],',
                '    }',
                '',
                '',
                'if __name__ == "__main__":',
                '    # Example: scan a /24 subnet',
                ...
                '',
                '    print(f"\\n--- Scan Report ---")',
                '    print(f"Alive: {report[\'alive_hosts\']}/{report[\'total_hosts\']}")',
    ...
    private isAutoGeneratedUntitled (doc: EditorDocument): boolean {
        // No path means it's in-memory only — definitely untitled
        if (!doc.path) {
            return !!doc.name && /^untitled/i.test(doc.name)
        }
        // Match common untitled naming patterns
        if (!/^untitled(-\d+)?(\..+)?$/i.test(doc.name)) {
            return false
        }
        // If the file lives inside the managed workspace folder, treat as auto-generated.
        const folder = this.getAutosaveTargetFolder()
        if (doc.path.startsWith(folder + path.sep) || doc.path.startsWith(folder + '/')) {
            return true
        }
        // Otherwise check if the path's basename directory is the autosave folder
        // (covers edge cases where path resolution differs)
        try {
            const parentDir = path.dirname(doc.path)
            return this.isSameFsPath(parentDir, folder)
        } catch {
            return false
        }
    }
    ...
    private async saveDocument (doc: EditorDocument): Promise<boolean> {
        ...
        if (initialPath && !this.isAutoGeneratedUntitled(doc)) {
        ...
        try {
            ...
            if (newPath) {
                ...
                if (this.selectedFilePathKeys.size && oldPath) {
                    const oldKey = this.getFsPathKey(oldPath)
                    const newKey = this.getFsPathKey(newPath)
                    if (oldKey && newKey && this.selectedFilePathKeys.has(oldKey)) {
                        const next = new Set(this.selectedFilePathKeys)
                        next.delete(oldKey)
                        next.add(newKey)
                        this.selectedFilePathKeys = next
    ...
    private registerEditorShortcuts (editor: any): void {
        ...
        editor.addCommand(KeyMod.CtrlCmd | KeyCode.KeyX, () => this.cutSelection(), 'editorTextFocus')
        editor.addCommand(KeyMod.CtrlCmd | KeyCode.KeyC, () => this.copySelection(), 'editorTextFocus')
        editor.addCommand(KeyMod.CtrlCmd | KeyCode.KeyV, () => this.pasteClipboard(), 'editorTextFocus')
        editor.addCommand(KeyMod.CtrlCmd | KeyCode.KeyA, () => this.selectAllText(), 'editorTextFocus')
