...
export class ProfilesService {
    ...
    */
    async duplicateProfile (profile: PartialProfile<Profile>, options?: { name?: string, group?: string }): Promise<PartialProfile<Profile>> {
        const copy = deepClone(profile)
        delete copy.id
        copy.isBuiltin = false
        copy.isTemplate = false
        copy.group = options?.group ?? profile.group
        copy.name = options?.name ?? this.translate.instant('{name} copy', profile)

        await this.newProfile(copy)
        await this.duplicateProfileSecrets(profile, copy)
    ...
    */
    async duplicateProfileSecrets (source: PartialProfile<Profile>, target: PartialProfile<Profile>): Promise<void> {
        const sourceProfile = this.getConfigProxyForProfile(source)
        const targetProfile = this.getConfigProxyForProfile(target)
        const provider = this.providerForProfile(sourceProfile) as ProfileProvider<Profile>|null
        if (!provider?.duplicateProfile) {
            return
        }

        try {
            await provider.duplicateProfile(sourceProfile, targetProfile)
        } catch (error) {
            console.warn(`Could not duplicate provider-managed data for profile "${source.name}"`, error)
        }
    }
    ...
    */
    async writeProfile (profile: PartialProfile<Profile>): Promise<void> {
        ...
        if (cProfile) {
            ...
            // Allow persisting updates for built-in profiles by storing an override entry
            this.config.store.profiles.push(profile)
        }
    }
