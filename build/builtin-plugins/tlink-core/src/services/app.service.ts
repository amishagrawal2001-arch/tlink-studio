...
export class AppService {
    ...
    private windowCloseInProgress = false
    ...
    openNewTab <T extends BaseTabComponent> (params: NewTabParameters<T>, direction: SplitDirection = 'r'): T {
        ...
        // If the active tab is already in a split, add the new tab to that split
        const activeTab = this.activeTab
        if (activeTab && activeTab.parent instanceof SplitTabComponent) {
            const parentSplit = activeTab.parent
            // Add the new tab relative to the active tab with the specified direction
            parentSplit.addTab(tab, activeTab, direction)
            return tab
        }
