...
import { ConfigService, HostAppService, Profile, SelectorService, ProfilesService, PromptModalComponent, PlatformService, BaseComponent as CoreBaseComponent, PartialProfile, ProfileProvider, TranslateService, Platform, ProfileGroup, PartialProfileGroup, QuickConnectProfileProvider, NotificationsService } from 'tlink-core'
...
export class ProfilesSettingsTabComponent extends BaseComponent {
    ...
    async newProfile (base?: PartialProfile<Profile>): Promise<void> {
        ...
        if (!base.isTemplate) {
            await this.profilesService.duplicateProfileSecrets(base, result)
        }
    ...
    async setProfileLogDirectory (profile: PartialProfile<Profile>): Promise<void> {
        ...
        try {
            ...
            const updatedProfile = deepClone(profile)
            updatedProfile.sessionLog = {
                enabled: updatedProfile.sessionLog?.enabled ?? true,
                directory,
                filenameTemplate: updatedProfile.sessionLog?.filenameTemplate,
                append: updatedProfile.sessionLog?.append,
            }
            await this.profilesService.writeProfile(updatedProfile)
            await this.config.save()
        } catch (error) {
