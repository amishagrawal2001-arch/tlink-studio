...
export class ConfigService {
    ...
    private migrate (config) {
        ...
        if (config.version < 7) {
            if (!config.configSync?.host || config.configSync?.host === 'https://api.tlink.sh') {
                config.configSync ??= {}
        ...
        if (config.version < 9) {
            ...
            const matchesDefaultKex = (list: string[]|undefined): boolean => {
                if (!Array.isArray(list) || list.length !== oldDefaultKex.length) {
                    return false
                }
                const sortedList = [...list].sort()
                const sortedDefault = [...oldDefaultKex].sort()
                return sortedList.every((value, index) => value === sortedDefault[index])
            }

            if (matchesDefaultKex(config.profileDefaults?.ssh?.options?.algorithms?.kex)) {
                config.profileDefaults.ssh.options.algorithms.kex = [...newDefaultKex]
            }
            ...
            for (const p of config.profiles ?? []) {
                ...
                if (matchesDefaultKex(p.options?.algorithms?.kex)) {
                    p.options.algorithms.kex = [...newDefaultKex]
                }
            }
            config.version = 9
        }
