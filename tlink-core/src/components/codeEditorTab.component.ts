...
export class CodeEditorTabComponent extends BaseTabComponent implements AfterViewInit {
    ...
    private isAutoGeneratedUntitled (doc: EditorDocument): boolean {
        if (!doc.path || !/^Untitled-\d+$/i.test(doc.name)) {
            return false
        }
        // Only treat as auto-generated if the file lives inside the
        // managed workspace folder.  Files opened from disk that happen
        // to match the pattern should save in place normally.
        const folder = this.getAutosaveTargetFolder()
        return doc.path.startsWith(folder + path.sep) || doc.path.startsWith(folder + '/')
    }
