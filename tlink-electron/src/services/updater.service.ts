...
const UPDATES_URL = 'https://api.github.com/repos/eugeny/tlink/releases/latest'
...
export class ElectronUpdaterService extends UpdaterService {
    ...
    constructor (
        ...
        config.ready$.toPromise().then(() => {
            if (config.store.enableAutomaticUpdates && this.electronUpdaterAvailable && !process.env.TLINK_DEV) {
    ...
    async update (): Promise<void> {
        if (!this.electronUpdaterAvailable) {
            ...
            if ((await this.platform.showMessageBox(
                {
                    type: 'warning',
                    message: this.translate.instant('Installing the update will close all tabs and restart Tlink.'),
                    buttons: [
