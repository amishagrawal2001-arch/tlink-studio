...
import { PlatformService, ClipboardContent, Platform, MenuItemOptions, MessageBoxOptions, MessageBoxResult, DirectoryUpload, FileUpload, FileDownload, DirectoryDownload, FileUploadOptions, wrapPromise, TranslateService, FileTransfer, PlatformTheme } from 'tlink-core'
...
export class ElectronPlatformService extends PlatformService {
    ...
    async saveConfig (content: string): Promise<void> {
        await wrapPromise(this.zone, this.hostApp.saveConfig(content))
    }
...
class ElectronDirectoryDownload extends DirectoryDownload {
    ...
    async createFile (relativePath: string, mode: number, size: number): Promise<FileDownload> {
        ...
        return new DirectoryFileDownload(fileDownload, this)
    }
    ...
    recordProgress (bytes: number): void {
        ...
        if (total > 0 && this.getCompletedBytes() >= total) {
            this.setCompleted(true)
        }
    }
}

class DirectoryFileDownload extends FileDownload {
    constructor (
        private inner: ElectronFileDownload,
        private parent: ElectronDirectoryDownload,
    ) {
        super()
    }
    ...
    async write (buffer: Uint8Array): Promise<void> {
        ...
        this.parent.recordProgress(buffer.length)
    }
